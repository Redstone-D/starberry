...
impl OAuthLayer {
    ...
    pub fn use_db(mut self, pool: starberry_core::sql::pool::SqlPool, expiration_seconds: u64) -> Self {
            ...
            if path_only == authorize_path {
                ...
                if method == HttpMethod::GET {
                    ...
                    // Rate limit GET authorize per client_id
                    {
                        ...
                        if !bucket.consume(1) {
                            ctx.response = return_status(StatusCode::TOO_MANY_REQUESTS);
                            return ctx;
                        }
                    }
                    ...
                    let client = if let Ok(c) = client_res { c } else {
                        ctx.response = return_status(StatusCode::BAD_REQUEST);
                        return ctx;
                    };
...
<form method=\"POST\" action=\"{}\"> 
    ...
    <button type=\"submit\" name=\"action\" value=\"deny\">Deny</button>
                    ...
                    ctx.response = html_response(html)
                    ...
                    let (client_id_post, code_challenge_post) = {
                        ...
                        let cookie_token = ctx.get_cookie("csrf_token").map(|c| c.value.clone()).unwrap_or_default();
                        // Parse form data
                        let f = ctx.form_or_default().await;
                ...
                let ip = match ctx.reader.get_ref().peer_addr() {
                ...
                let token_opt = ctx.meta().header.get("authorization")
                    .map(|hv| hv.as_str())
                    .and_then(|s| s.strip_prefix("Bearer ").map(str::to_string));
                let token_str = if let Some(t) = token_opt {
                    t
                } else {
                    ctx.response = return_status(StatusCode::UNAUTHORIZED);
                    return ctx;
                };
