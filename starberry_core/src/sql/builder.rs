...
impl<'q> SqlQuery<'q> {
    ...
    /// Execute and fetch one row using an async SqlPool.
    pub async fn fetch_one_pool(self, pool: &SqlPool) -> Result<HashMap<String, String>, DbError> {
        let rows = self.fetch_all_pool(pool).await?;
        rows.into_iter().next().ok_or_else(|| DbError::QueryError("Expected one row".into()))
    }
    ...
    /// Execute and map all rows via FromRow using an async SqlPool.
    pub async fn fetch_all_as_pool<T: FromRow>(self, pool: &SqlPool) -> Result<Vec<T>, DbError> {
        let rows = self.fetch_all_pool(pool).await?;
        rows.into_iter().map(|row| T::from_row(&row)).collect()
    }

    /// Execute and map the first row via FromRow using an async SqlPool.
    pub async fn fetch_one_as_pool<T: FromRow>(self, pool: &SqlPool) -> Result<T, DbError> {
